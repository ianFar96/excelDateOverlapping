...
func GetCarbonDates(date string, startTime string, endTime string) (*carbon.Carbon, *carbon.Carbon, error) {
	...
	minutes, err := strconv.ParseInt(splittedStartTime[1], 10, 64)
	if err != nil {
		message := fmt.Sprintf("Wrong start time format, expected hh:mm given %s: %s", startTime, err)
		fmt.Println(message)
		return nil, nil, errors.New(message)
	...
	startDate := carbon.CreateFromDateTimeMilli(int(year), int(month), int(day), int(hour), int(minutes), 0, 0)
	...
	minutes, err = strconv.ParseInt(splittedEndTime[1], 10, 64)
	if err != nil {
		message := fmt.Sprintf("Wrong end time format, expected hh:mm given %s: %s", endTime, err)
		fmt.Println(message)
		return nil, nil, errors.New(message)
	...
	endDate := carbon.CreateFromDateTimeMilli(int(year), int(month), int(day), int(hour), int(minutes), 0, 0)
